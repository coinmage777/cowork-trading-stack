"""
Variational + Aster Volume Farmer — same-symbol delta-neutral hedge.

Variational LONG/SHORT  (wallet 0xWALLC..., session-based) — RFQ pricing
  +
...
Per cycle:
  ...
  3) asyncio.gather(open_var, open_ast) near-simultaneous entry
  4) verify both legs have non-zero positions within 5s, else emergency-close + STOP
  5) hold ``interval_sec`` (jitter ±20%)
  6) asyncio.gather(close_var, close_ast) reduce-only
  7) verify both flat, journal the cycle

Kill switches:
  - File data/KILL_VAR_ASTER_FARMER exists
  - Daily PnL  < VAR_ASTER_FARMER_DAILY_STOP_USD (default -$3)
  - Daily cap reached (default 20 round-trips)
  - 5 consecutive failures
  - Either venue collateral below floor

Funding-aware mode (optional):
  - Variational does NOT expose funding_rate via wrapper (RFQ pricing → no separate funding feed).
  - Aster exposes lastFundingRate via /fapi/v3/premiumIndex.
  - With only one funding signal, true funding-arb is NOT possible. We instead use the Aster
    funding rate as a tilt: if VAR_ASTER_FARMER_FUNDING_AWARE=true and Aster funding is < 0
    (shorts pay longs), we bias the LONG leg to whichever venue is more volume-yielding.
    Default OFF — direction is random or fixed.

Symbol mapping:
  - asset "BTC"  → Variational symbol "BTC"   |  Aster symbol "BTCUSDT"
  - asset "ETH"  → Variational symbol "ETH"   |  Aster symbol "ETHUSDT"

Architecture note:
  Both wrappers are async-friendly (Variational uses HTTP polling; Aster uses urllib in a thread
  via run_in_executor — wrapper is sync-blocking but called from async). They run in-process
  via mpdex.factory.create_exchange and direct AsterExchange().init().

Usage:
  cd <INSTALL_DIR>/multi-perp-dex
  source main_venv/bin/activate
  python -m strategies.var_aster_farmer --dry-run
  python -m strategies.var_aster_farmer --live
"""
...
logger = logging.getLogger("var_aster_farmer")
...
VAR_ACCOUNT_NAME = "variational_2"
...
# Variational + Aster ETH liquidity has been observed; BTC default for safety.
...
class FarmerConfig:
    position_size_usd: float = field(
        default_factory=lambda: _env_float("VAR_ASTER_FARMER_POSITION_SIZE_USD", 50.0)
    )
    leverage: int = field(default_factory=lambda: _env_int("VAR_ASTER_FARMER_LEVERAGE", 3))
    interval_sec: int = field(default_factory=lambda: _env_int("VAR_ASTER_FARMER_INTERVAL_SEC", 300))
    daily_cap: int = field(default_factory=lambda: _env_int("VAR_ASTER_FARMER_DAILY_CAP", 10))
    daily_stop_usd: float = field(
        default_factory=lambda: _env_float("VAR_ASTER_FARMER_DAILY_STOP_USD", -3.0)
    )
    kill_file: str = field(
        default_factory=lambda: os.environ.get(
            "VAR_ASTER_FARMER_KILL_FILE", "data/KILL_VAR_ASTER_FARMER"
        )
    )

    # Asset rotation
    asset_pool: tuple = field(
        default_factory=lambda: _env_list("VAR_ASTER_FARMER_ASSET_POOL", ASSET_POOL_DEFAULT)
    )
    asset_rotation: bool = field(default_factory=lambda: _env_bool("VAR_ASTER_FARMER_ASSET_ROTATION", True))
    direction_random: bool = field(default_factory=lambda: _env_bool("VAR_ASTER_FARMER_DIRECTION_RANDOM", True))

    # Funding-aware (Aster funding only — Variational has no per-symbol funding feed)
    funding_aware: bool = field(default_factory=lambda: _env_bool("VAR_ASTER_FARMER_FUNDING_AWARE", False))
    funding_diff_threshold: float = field(
        default_factory=lambda: _env_float("VAR_ASTER_FARMER_FUNDING_DIFF_THRESHOLD", 0.0005)
    )

    # Jitter
    size_jitter_pct: float = field(default_factory=lambda: _env_float("VAR_ASTER_FARMER_SIZE_JITTER_PCT", 0.10))
    hold_jitter_pct: float = field(default_factory=lambda: _env_float("VAR_ASTER_FARMER_HOLD_JITTER_PCT", 0.20))
    cycle_gap_min: float = field(default_factory=lambda: _env_float("VAR_ASTER_FARMER_CYCLE_GAP_MIN", 2.0))
    cycle_gap_max: float = field(default_factory=lambda: _env_float("VAR_ASTER_FARMER_CYCLE_GAP_MAX", 8.0))
    hold_min: int = field(default_factory=lambda: _env_int("VAR_ASTER_FARMER_HOLD_MIN", 180))
    hold_max: int = field(default_factory=lambda: _env_int("VAR_ASTER_FARMER_HOLD_MAX", 600))

    # Hard limits
    min_var_collateral: float = field(
        default_factory=lambda: _env_float("VAR_ASTER_FARMER_MIN_VAR_COLLATERAL", 80.0)
    )
    min_ast_collateral: float = field(
        default_factory=lambda: _env_float("VAR_ASTER_FARMER_MIN_AST_COLLATERAL", 50.0)
    )
    max_spread_pct: float = field(default_factory=lambda: _env_float("VAR_ASTER_FARMER_MAX_SPREAD_PCT", 0.005))
    fill_timeout_sec: float = field(default_factory=lambda: _env_float("VAR_ASTER_FARMER_FILL_TIMEOUT_SEC", 5.0))
    max_consecutive_failures: int = field(
        default_factory=lambda: _env_int("VAR_ASTER_FARMER_MAX_CONSEC_FAILS", 5)
    )
    close_verify_timeout: float = field(
        default_factory=lambda: _env_float("VAR_ASTER_FARMER_CLOSE_VERIFY_TIMEOUT", 6.0)
    )

    # Allow override of pre-flight reconcile-blocked behavior — STRONGLY discouraged
    skip_reconcile_block: bool = field(default_factory=lambda: _env_bool("VAR_ASTER_FARMER_SKIP_RECONCILE_BLOCK", False))


class VarAsterFarmer:
    def __init__(self, cfg: FarmerConfig, dry_run: bool = False):
        ...
        self._journal_path = DATA_DIR / "var_aster_farmer.jsonl"
        self._asset_idx = 0
    ...
    async def connect(self):
        ...
        # ---- Variational_2 in-process via factory --------------
        var_keys = cfg_yaml["exchanges"][VAR_ACCOUNT_NAME]["keys"]
        var_kp = SimpleNamespace(
            evm_wallet_address=var_keys["evm_wallet_address"],
            evm_private_key=var_keys["evm_private_key"],
            session_cookies=var_keys.get("session_cookies", ""),
        )
        logger.info("connecting Variational_2 (wallet=%s)...", var_kp.evm_wallet_address)
        self.var = await create_exchange("variational", var_kp)
        ...
        logger.info("Variational + Aster connected")
    ...
    async def reconcile(self) -> bool:
        ...
        currently holding a position on Variational_2 / Aster for any of our pool assets,
        reconcile will abort. Override only with VAR_ASTER_FARMER_SKIP_RECONCILE_BLOCK=1
    ...
    async def _close_both(self, asset: str) -> Dict[str, Any]:
        ...
        try:
            await asyncio.gather(
                self.var.close_position(self._var_symbol(asset), vpos)
    ...
    async def _emergency_close(self, asset: str):
        ...
        tasks = []
        if vpos:
            tasks.append(self.var.close_position(self._var_symbol(asset), vpos))
        if apos:
...
async def _amain():
    ...
    farmer = VarAsterFarmer(cfg, dry_run=args.dry_run)
