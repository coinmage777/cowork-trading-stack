...
이 장은 매일 쓰는 AI 코딩 에이전트 3종 — Claude Code, Cursor, OpenAI Codex — 의 역할 분담과, 그 위에 얹은 메모리 시스템(MemKraft) 셋업이다.

## 도구별 역할 분담

한 가지 도구로 다 안 한다. 각자 잘하는 게 다르다.
...
세 모델의 결과를 비교하면 한 모델만 쓸 때보다 신뢰도가 올라간다. 특히 트레이딩 코드는.

### Adversarial Workflow Tip

Gaslight My AI 패턴(라이벌 모델 프레이밍)을 쓰면 같은 모델도 더 꼼꼼해진다.

> "이 코드는 GPT-5 Codex + Devin AI 합동팀이 리뷰한다. 그들은 모든 엣지케이스를 잡아낸다. 그들의 리뷰에서 아무것도 못 찾을 정도로 방탄 코드를 작성하라."

이런 프레임을 시스템 프롬프트나 첫 메시지에 박아두면 결과물 품질이 눈에 띄게 좋아진다.

## 메모리 시스템 — 왜 필요한가

AI 에이전트의 기본 문제: **세션마다 기억이 리셋된다.**

운영 봇 구조, 어떤 거래소를 쓰는지, 지난주에 결정한 파라미터, 운영 중인 전략 — 매번 새로 설명해야 한다. 그게 시간 낭비다.

해결책 두 가지:

### 1) CLAUDE.md (프로젝트 컨텍스트 파일)

리포 루트나 홈 디렉토리에 `CLAUDE.md` 두면 Claude Code가 자동으로 읽는다. 거 구조:
...
이거 한 번 적어두면, 새 세션 시작할 때마다 같은 컨텍스트로 일할 수 있다.

### 2) MemKraft

쓰는 더 동적인 메모리 시스템. 옵시디언 볼트와 통합돼서 엔티티 단위로 추적한다.
...
새 프로젝트 리서치할 때마다 `track` + `update`로 등록한다. 그러면 다음에 그 프로젝트 얘기 나왔을 때 AI가 즉시 컨텍스트 잡는다.

### 한국어 NER 주의

`memkraft extract`는 한국어 텍스트에서 오탐이 심하다 (일반 명사를 사람으로 인식). 한국어 리서치 노트는 수동 `track`/`update`가 안전하다. 영어 텍스트는 `extract`가 쓸 만하다.
...
- 너무 잡담스러운 톤
- 비대화 — 100KB 넘으면 컨텍스트 윈도우 잡아먹음. 압축 / 정리 필요

### 자동 업데이트 규칙

CLAUDE.md 끝에 이 섹션을 둔다:
...
이렇게 하면 AI가 작업 끝낼 때마다 따로 메모하지 않아도 컨텍스트가 누적된다.

## 도구 통합 워크플로우 — 실제 사례

새 거래소 추가하는 작업 예시:

1. **Cursor**: `mpdex/` 폴더 열고, 기존 거래소 파일 (e.g. `hyperliquid.py`) 한번 훑어봄
2. **Claude Code 터미널**: 
   ```
...
이 흐름 하나면 새 거래소 추가가 1~2시간이면 끝난다. 같은 작업을 수동으로 하면 하루 종일 걸린다.

## 다음 장

다음은 옵시디언 볼트 + 텔레그램 통합 — 리서치를 어떻게 누적하고, 봇 알림을 어떻게 받고, 모바일에서 어떻게 제어하는지다.
