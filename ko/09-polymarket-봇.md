...
- 가격 = 확률 (0~1 USD)
- 사건 발생 시 정답 토큰이 1 USD로 정산, 오답은 0
- 트럼프 당선 시장 같은 거대 마켓부터, "이번 주 ETH 가격" 같은 작은 것도 있음

### Polymarket 데이터의 특이점

- **CLOB (중앙 오더북)** — Polygon 위에 있지만 매칭은 오프체인 / 정산은 온체인
- **수수료 없음** (USDC 매수/매도 자체 수수료. 가스비는 약간)
- **유동성** 마켓에 따라 천차만별 — 트럼프 시장은 깊고, 작은 마켓은 얕음
...
- **시장이 비효율적** — Polymarket은 크립토 트레이더 + 정치 베팅 사람들이 섞여서 흥미로움
- **명확한 outcome** — 가격이 0 또는 1로 수렴 (정산일에)
- **무한정 보유 가능** — 청산 없음, 펀딩비 없음
- **차익 기회** — 다른 예측 시장 (Predict.fun, Kalshi)과 가격 차이

## 왜 어려운가 (망한 케이스)

Polymarket 서브계정 하나로 8일 만에 약 60% 떨어진 적 있다. 그 과정에서 배운 것들.

### 1) 역선택 (Adverse Selection)

- mid-price에 리밋 매수 주문 박음
- 그 가격에서 체결되려면 누가 그 가격에 매도해야 함
- 매도자는 봇 / 정보 우위가 있는 사람
- → 체결되는 주문은 봇 예측이 틀린 케이스

데이터: 체결률 14%, 체결 시 승률 29%. 즉, 체결 자체가 마이너스 시그널.

**교훈**: 단순 방향성 베팅을 리밋 주문으로 하면 마켓메이킹이나 정보 우위 없이는 거의 불가능.
...
**해결**: `/price?side=buy` API 사용해서 실제 매수 가능 가격 조회.
...
**교훈**: **절대 주문 후 cancel 하지 말 것.** 대신 정산 시점에 `get_order()`로 사후 확인.
...
DB의 `size`는 달러 비용인데 공식이 shares처럼 계산.
- 이전 (잘못): `WIN = (1-ep) * size * 0.9`
- 수정: `WIN = size * (1/ep - 1)`

이 버그로 DB PnL이 실제 잔고 변화와 크게 달랐다. **DB PnL 신뢰 X. 온체인 잔고가 진실의 원천**.

### 6) Predict.fun 가스 부족

Signer EOA에 BNB 없으면 클레임 실패. Predict Account가 아니라 **Signer 주소**에 BNB 필요.
...
2. **predict_snipe** (Predict.fun): 임박 만료 + 변동성 기반 가격 모델로 가격이 잘못된 경우 진입

### Shadow 전략

`expiry_snipe`는 라이브 비활성화, shadow에서만 추적 (DB에 기록만, 실제 주문 X). 데이터 누적 후 패턴 발견하면 다시 라이브 검토.
...
Predict.fun은 BSC 기반 예측 시장. 비슷한 패턴이지만 차이점:
...
이 모델 + 자산별 edge buffer (BTC 0%, ETH 3%, SOL 3%, BNB 2%) 로 진입 결정.
...
다음은 Gold cross-exchange arb — 금 ETF / 토큰화 금 / Perp 금 시장에서의 차익. 일반화된 cross-exchange 패턴.
