...
김프 (Kimchi Premium) — 한국 거래소(업비트, 빗썸, 코인원 등)에서 비트코인이나 다른 코인 가격이 글로벌 거래소(바이낸스, OKX 등)보다 높게 거래되는 현상입니다.

이 차이를 차익으로 만드는 방법을 정리합니다.
...
[kimpga.com](https://kimpga.com), [coinview.io](https://coinview.io) 같은 사이트에서 실시간 확인이 가능합니다.
...
**문제**: 한국 거래소에서 USD/USDT 매수 / 출금이 안 됩니다. KRW를 들고 나가는 채널이 제한적입니다.
...
이것은 시간 차가 길고, 김프가 음수일 때만 가능합니다 (드물고 짧음).

### 보는 현실

순수 김프 차익은 어렵습니다. **자본 흐름의 비대칭** 때문입니다. 한국 → 글로벌이 어려워서 차익 기회가 살아남는 것입니다. 그 어려움을 운영자도 똑같이 겪습니다.

그래서 보는 더 실용적 형태:

## Cross-Venue 차익 — 더 일반화된 형태

김프는 cross-venue 차익의 한 케이스입니다. 더 일반적으로:
...
이 패턴은 백테스트만 해보고 라이브로는 굴려보지 않았습니다. 큰 무브 빈도가 낮고, 진입 시 양쪽 동시 체결이 어려워서입니다.

## 김프 자동화 — 현실적 시나리오

김프 차익을 제대로 자동화하려면 사실상 두 사람 시스템이 효율적입니다:

- **글로벌 사이드 운영자**: USDT → 코인 매수 → 한국 전송
- **한국 사이드 운영자**: 코인 받기 → KRW 매도 → KRW 들고 다음 라운드 대기

혼자 하면 KRW를 다시 USDT로 바꾸는 채널이 병목입니다.

### 실제 운영하는 방식

본격 김프 봇은 굴리지 않습니다. 대신 **김프가 비정상적으로 높을 때** 알림 → 수동 판단으로 들어갑니다. 자주 있는 일이 아니라 자동화 ROI가 나오지 않습니다.
...
이 정도가 김프에 대한 자동화의 실용적 최대치입니다.

## 더 현실적인 차익 자동화: 펀딩 모니터

더 자주 쓰는 것은 펀딩비 모니터입니다:
...
이것을 매 5분마다 돌리면 펀딩 차익 기회 알람을 받습니다. 들어갈지 말지는 판단입니다.

## 다음 장

다음은 백테스팅 — 전략을 라이브에 박기 전에 어떻게 검증하는가. Minara 기반 워크플로우입니다.
