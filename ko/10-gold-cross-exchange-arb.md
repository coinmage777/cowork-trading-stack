...
굴리는 차익 패턴 중에서 일반화 가치가 있는 것 — 토큰화된 자산이나 동일 자산이 여러 거래소에서 다른 가격에 거래될 때입니다.

이름이 "Gold"인 이유는 처음 적용한 곳이 토큰화 금(PAXG, XAUT 등)이었기 때문입니다. 같은 패턴이 다른 자산에도 적용됩니다.
...
- 거래소별로 가격이 다름 (유동성 / 수요 차이)
- 크립토 vs TradFi 시간 갭 (TradFi는 주말 휴장)
- 펀딩비 / 보관 수수료 차이

이 차이 중 **거래 가능한 것**을 자동화로 잡는 것이 핵심입니다.

## 쓰는 일반 패턴

### Pattern 1: 동일 자산 거래소간

같은 자산 A가 거래소 X에서는 $100, 거래소 Y에서는 $99에 거래됩니다.
...
이것은 펀딩비가 안정적으로 양수인 경우 거의 무위험입니다. **carry trade**입니다.
...
이것은 백테스트를 충분히 해야 합니다. TradFi 갭이 항상 채워지는 것은 아닙니다 (큰 뉴스 시 안 채워짐).
...
단, LBMA 가격은 24시간 거래가 안 됩니다. 비교 데이터에 한계가 있습니다.
...
이 차익의 가장 큰 위험은 **한쪽만 체결되는 경우**입니다. 그러면 갑자기 단방향 노출이 생깁니다.
...
오더북 mid price만 보면 안 됩니다. 사이즈를 흡수하는 데 슬리피지가 발생합니다.
...
거래소 N개에 자본을 어떻게 나눌지에 대한 룰:
- 각 거래소에 최소 사이즈의 3~5배 자본
- 페어 트레이딩이라 한쪽이 마이너스가 나도 다른 쪽이 플러스 → 청산 위험 작음
- 하지만 한 거래소에서 출금이 정지되면 큰 문제 → 한 거래소에 자본을 너무 몰지 마세요
- 룰: 한 거래소 최대 = 총 자본의 25%
...
차익 봇은 한 번 진입하면 수렴까지 보유합니다. 그동안 이상 상황 감지가 중요합니다:
...
- 작은 거래소 차익 도전: 한쪽 체결 후 다른 쪽 cancel → ghost position. 사이즈를 줄여야 함
- TradFi 시간차 베팅: 큰 뉴스 시 갭이 안 채워짐. 백테스트를 너무 일반화함
- 토큰화 금 차익: 유동성이 너무 얕아서 실용적 사이즈가 안 됨

## 결론

Cross-exchange 차익은 매력적이지만, 자본 효율 / 운영 복잡도 / 거래소 리스크 트레이드오프가 큽니다. 메인 전략으로는 쓰지 않고 보조 전략으로만 굴립니다.

**메인 전략은 페어 트레이딩 (한 거래소 안에서 BTC vs ETH)이 자본 효율과 운영 단순성에서 우위입니다.**

차익은 시장이 비효율적인 특정 시점에만 알람으로 잡고, 수동으로 들어가는 것도 한 방법입니다.

## 다음 장

다음은 운영 인프라 + 원칙입니다. VPS, 모니터링, 로깅, 백업, 재시작, 보안 — 봇이 24/7 돌아가게 하는 것입니다.
