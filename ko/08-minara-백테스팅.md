...
전략을 라이브에 박기 전에 백테스트를 하지 않으면 그냥 도박꾼입니다. 사용하는 백테스팅 워크플로우와 그 과정에서 배운 것들입니다.

## 왜 백테스트가 중요한가

백테스트를 하지 않고 라이브에 박았다가 장기간 누적 손실을 본 경험이 있습니다. 원인은 TP/SL 비율이 본전 승률 94%를 요구하는 구조였는데, 그것을 라이브 데이터로 알아챘다는 점입니다. 백테스트를 한 시간만 했어도 그 자리에서 발견됐을 문제입니다.
...
- **개선**: tick 데이터 또는 1초봉. 그것이 안 되면 1분봉의 high/low를 양쪽 다 시뮬
...
거래소 API에 rate limit이 있으니 sleep이 필요합니다.
...
OOS PnL이 IS PnL보다 명백히 떨어지면 과적합입니다. 다시 디자인합니다.

### 5단계: 시뮬레이션 → 페이퍼 트레이드 → 라이브

백테스트 통과 → 페이퍼 트레이드 (실시간 가격, 가짜 잔고) 1~2주 → 소액 라이브 → 확인 → 스케일.

페이퍼와 백테스트 결과가 크게 다르면 데이터 / 시그널 / 슬리피지 가정 중 하나가 틀린 것입니다.

## Minara — 사용하는 백테스팅 도구

멀티 거래소 페어 트레이딩 봇용으로 빌드한 백테스팅 모듈을 Minara로 부릅니다. 기본 기능:
...
이 모두 백테스트에서 먼저 확인하고 라이브에 적용한 것입니다.

## 다음 장

다음은 [Polymarket](https://polymarket.com/?ref=coinmage) 봇 — 예측 시장 자동매매. Perp 봇과 완전히 다른 시장 구조입니다.
