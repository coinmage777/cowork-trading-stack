...
메인 봇이 굴리는 핵심 전략입니다. 여러 Perp DEX에서 BTC/ETH 같은 페어를 동시에 long-short으로 들어가서, 두 자산의 상대적 움직임에서 알파를 짜내는 방식입니다.
...
크립토에서 가장 흔한 페어는 **BTC vs ETH**입니다. 둘 다 시장 베타가 비슷하고, 상관도는 평소 0.8 이상입니다.

### 왜 단방향 매매보다 유리한가

- **시장 노출 헷지** — 비트코인이 갑자기 10% 빠져도 양쪽이 같이 빠지므로 PnL 영향이 작습니다
- **레버리지 안정적으로 사용 가능** — 단방향 10x는 청산 위험이 크지만, 헷지된 페어 10x는 더 안전합니다
- **백테스트 가능** — 두 자산의 가격 비율 시계열만 있으면 시그널 디자인이 가능합니다

### 단점

- 복잡함 — 한쪽만 들어가는 것보다 변수가 두 배입니다
- 교차 거래소 운영하면 자본 / API / 모니터링이 두 배가 됩니다
- 갑자기 한 자산만 movement (ETH alpha unwind 같은) 발생하면 큰 손실이 발생합니다

## 운영 봇 구조 — 개요

운영해온 멀티 Perp 페어 트레이딩 봇 (`mpdex` 프레임워크 위) 구조입니다:
...
새 거래소 추가가 쉬워야 합니다. Factory 패턴 + lazy loading 방식입니다:
...
이렇게 하면 `config.yaml`에 거래소 이름만 적으면 자동 로드됩니다.

### 통합 인터페이스

모든 거래소가 같은 메소드를 노출합니다:
...
근거: 단기 모멘텀은 평균 회귀하는 경향이 강합니다. 페어에서 이 효과가 단방향보다 안정적입니다.
...
메인 시그널은 위 두 개입니다. 그 외:
- 볼린저 밴드: 스프레드가 2σ 밖
- RSI 발산: BTC RSI 70+ + ETH RSI 50- 같은 비대칭

이 보조 시그널들은 가중치를 낮게 둡니다 (0.10–0.20).

### 4) 레짐 필터 — 매우 중요

상관관계가 낮은 시점에는 페어 트레이딩이 통하지 않습니다 (둘이 따로 움직이면 평균 회귀 가설이 깨짐).
...
이 필터를 추가한 후 일일 손실 빈도가 명확히 줄어들었습니다.

### 5) 방향 비대칭 — 데이터 기반

데이터: ETH long (coin2_long) 승률 80% vs BTC long (coin1_long) 승률 70%.

이유 추측: 알트 시즌 / 베타 차이. 어쨌든 데이터가 그러면 따라야 합니다.
...
운영 봇이 청산을 결정할 때 체크하는 순서입니다 (위에서 아래로):

1. **하드 스톱** (-2.5% PnL): 무조건 즉시 청산
2. **트레일링 스탑**: 활성화 후 callback 도달
3. **고정 익절** (+2% PnL): 이전에는 0.4%였는데 R:R이 맞지 않아 5x 늘렸습니다
...
이 부분이 가장 비싸게 배운 것입니다. **TP / SL 비율이 손익분기 승률을 결정합니다**.

예: TP 0.4% + SL 3% → 본전 승률 = 3 / (0.4 + 3) = 88%. 맞추기 어려운 수치입니다.

이전 실수: TP 0.4% / SL 3%로 운영 → 장기간 누적 손실.

수정 후: TP 2% / SL 2.5% → 본전 승률 55%. 실제 승률 67–84% → 수익성 확보.

**TP / SL은 마진 기준이 아니라 R:R 기준으로 디자인해야 합니다**. 수학적으로 검증한 다음에 라이브 운영합니다.
...
진입 가격이 빗나갔을 때 추가 진입으로 평단을 낮추는 패턴입니다. **위험하지만 잘 쓰면 강력합니다**.
...
같은 시점에 여러 거래소에 동시 진입하면 시장 충격이 운영자에게 돌아옵니다. 그래서:
...
뒤 그룹일수록 강한 시그널만 진입 → 운영 봇이 동시 청산되는 경우를 줄입니다.

## Strategy Evolver

시그널 가중치를 자동으로 튜닝하는 컴포넌트입니다. 매 6시간마다:
1. 최근 트레이드 성과 분석
2. 시그널별 기여도 계산
3. 가중치 조정 (성과 좋은 시그널 ↑)
4. 새 가중치로 다음 사이클 시작
...
이것을 두지 않으면 evolver가 핵심 시그널을 0으로 만들어서 봇이 이상해집니다.

## 운영 팁

### 1) 처음에는 작게
페어 트레이딩 봇은 진입 시그널 / 청산 시그널 / 사이즈 / 레버리지 / 거래소 / 페어 / 시장 변동성 등 변수가 많습니다. 처음에는 작은 자본 + 한 거래소 + 한 페어로 시작하세요. 안정 운영을 확인한 후 확장합니다.

### 2) 데이터 기반 결정
"감으로 SL 좀 늘려볼까"는 금지입니다. DB에서 실제 손익 분포를 보고 결정하세요. 매번 변경 전에 N개 트레이드의 분포를 분석합니다.

### 3) 한 번에 하나만 변경
SL / TP / 사이즈 / 레버리지를 동시에 바꾸면 어느 것이 효과가 있는지 알 수 없습니다. A/B 테스트 식으로 하나씩 변경합니다.

### 4) 서킷브레이커 필수
일일 -X% 손실 시 자동 정지. 기본값 -$30~-$150 (자본 규모에 따라).

### 5) Telegram 통합
모든 진입 / 청산 / 에러를 텔레그램으로 전송합니다. 모바일에서도 봇 상태를 알 수 있게 합니다.
...
이것이 정확한 수익을 의미하지는 않습니다. 어떤 날은 손실, 어떤 날은 수익입니다. 시간이 지나면서 평균이 양수면 성공입니다. 매일 잔고 트래커로 진실을 확인합니다 (DB의 PnL 수치는 부정확할 수 있음 — 다음 장 포함).

## 다음 장

다음은 김프 + Cross-Venue 차익입니다. 한국 거래소와 글로벌 거래소 간의 가격 차이를 어떻게 자동화하는지 다룹니다.
