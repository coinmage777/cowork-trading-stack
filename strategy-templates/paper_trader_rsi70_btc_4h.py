"""
Live PAPER trader for the RSI(14) > 70 continuation strategy on BTCUSDT 4H
...
Journal: ../DH_bithumb_arb-main/strategies_minara/data/paper_rsi70_journal.jsonl
Log file: ../DH_bithumb_arb-main/strategies_minara/logs/paper_rsi70.log
"""
...
JOURNAL_PATH = DATA_DIR / "paper_rsi70_journal.jsonl"
LOG_PATH = LOG_DIR / "paper_rsi70.log"
...
# Logging
# ---------------------------------------------------------------------------
logger = logging.getLogger("paper_rsi70")
...
def handle_entry(state: State, bar_ts: int, close: float, rsi: float) -> None:
    ...
    msg = (f"[PAPER RSI70] ENTRY LONG ${NOTIONAL:.0f}\n"
...
def startup_row(state: State, ccxt_version: str) -> None:
    ...
    tg_send(f"[PAPER RSI70] startup pid={os.getpid()} pos_active={state.position.active} trades={state.cumulative.trades}")


def main() -> None:
    logger.info("paper_rsi70 starting - journal=%s log=%s", JOURNAL_PATH, LOG_PATH)
    state = restore_state()
