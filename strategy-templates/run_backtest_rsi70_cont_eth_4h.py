"""
Backtest RSI>70 Continuation ETH/USDT 4H on Binance USDM futures.

Purpose: check whether the RSI>70 continuation pattern that REPLICATES on BTC 4H
(n=148, WR 33%, MDD 15%, Sharpe 0.72) generalizes beyond BTC.

Strategy (identical to BTC version):
...
SYMBOL = "ETH/USDT:USDT"  # Binance USDM perp
TIMEFRAME = "4h"
YEARS = 6
...
def ytd_2026_stats(trades: list[dict]) -> dict:
    """2026 YTD regime check — does strategy still work in current regime?"""
...
def fmt_markdown(stats: dict, trades: list[dict], open_mtm: dict | None, rows: list[list], yearly: dict, ytd: dict) -> str:
    ...
    # BTC 4H baseline (already verified REPLICATES Minara's claim)
    btc = {
        "trades": 148,
        "wr_pct": 33.0,
        "mdd_pct": 15.0,
    ...
    # YES: key metrics (WR, PF, Sharpe) all in same ballpark as BTC AND profitable
    # PARTIAL: some metrics match, some don't, or marginally profitable
    # NO: clearly worse — unprofitable or far worse risk-adjusted returns
    ...
    md = []
    md.append("# RSI>70 Continuation ETH 4H — Generalization Test")
    md.append("")
    ...
    md.append("")
    md.append("## ETH vs BTC 4H Comparison")
    md.append("")
    md.append("| Metric | BTC 4H (baseline) | ETH 4H | Delta |")
    md.append("|---|---|---|---|")
    md.append(f"| Trades | {btc['trades']} | {actual_trades} | {actual_trades - btc['trades']:+d} |")
    md.append(f"| Win Rate | {btc['wr_pct']}% | {actual_wr}% | {actual_wr - btc['wr_pct']:+.2f}pp |")
    md.append(f"| Profit Factor | n/a | {actual_pf} | - |")
    md.append(f"| Sharpe (daily ann.) | {btc['sharpe']} | {actual_sharpe} | {actual_sharpe - btc['sharpe']:+.3f} |")
    md.append(f"| MDD | {btc['mdd_pct']}% | {actual_mdd}% | {actual_mdd - btc['mdd_pct']:+.2f}pp |")
    md.append(f"| Total Return | n/a | {actual_ret}% | - |")
    md.append("")
    ...
    md.append("")
    md.append(f"BTC 2026 YTD showed WR 18.8% / return -9.4% (regime warning). ETH comparison:")
    md.append("")
    for k, v in ytd.items():
        md.append(f"- **{k}**: {v}")
    md.append("")
    md.append("## Aggregate Stats (Full 6Y)")
    md.append("")
...
def main():
    ...
    out_md = HERE / "backtest_rsi70_cont_eth_4h.md"
    out_md.write_text(md, encoding="utf-8")
    print(f"Wrote {out_md}", flush=True)

    out_json = HERE / "backtest_rsi70_cont_eth_4h.json"
